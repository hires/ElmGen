...
/**
 * This class represents a program on the DSP. Effect programs should
 * subclass this class and call the appropriate methods to set up and
 * test the program.
 * 
 * @author andrew
 */
public class ElmProgram {
	...
	// skip flags
	public static final int SKP_NEG = 0x01;
	public static final int SKP_GEZ = 0x02;
	public static final int SKP_ZRO = 0x04;
	public static final int SKP_ZRC = 0x08;
	public static final int SKP_RUN = 0x10;	
	...
	public static final int CHO_SIN = 0x00;
	public static final int CHO_COS = 0x01;
	public static final int CHO_REG = 0x02;
	public static final int CHO_COMPC = 0x04;
	public static final int CHO_COMPA = 0x08;
	public static final int CHO_RPTR2 = 0x10;
	public static final int CHO_NA = 0x20;
	
	/**
	 * Creates a new ElmProgram with a blank memory map and
	 * instruction list. The default samplerate is 32768Hz.
	 * 
	...
	public void allocDelayMem(String name, int size) {
		...
		if((offset + size) > MAX_DELAY_MEM) {
			throw new ElmProgramException("not enough space to allocate memory segment " +
	 ...
	 */
	private MemSegment getDelayMemByName(String name) {
		String name2 = name.toUpperCase();
		for(int i = 0; i < memoryMap.size(); i ++) {
	 ...
	 */
	private void checkCodeLen() {
		if(instList.size() == MAX_CODE_LEN) {
	...
	/**
	 * Multiplies the base 2 absolute value of the ACC with the
	 * scale value and then adds an offset.
	 * 
	 ...
	 * @param nskip the number of instructions to skip if the conditions are met
	 */
	 ...
	 * @param scale the amount to scale the register by before adding it to the ACC
	 */
	 ...
	 * @param scale the amount to scale the ACC by after writing the register
	 */
	 ...
	 * @param scale the amount to scale the register value by before comparison
	 */
	 ...
	 * then scales the result by scale and finally adds the value of the register
	 * at addr to the result.
	 * 
	 * @param addr the register to use
	 * @param scale the amount to scale the result before adding the register
	 */
	 ...
	 * address pointer register. Scales the result by scale and adds to
	 * the ACC.
	 * 
	 * @param scale the amount to scale by before adding to the ACC
	 */
	 ...
	 * 
	 * @param addr the delay memory offset
	 * @param scale the amount to scale the ACC by after writing to delay memory
	 */
	...
	/**
	 * Reads from the delay memory with the read pointer value modulated
	 * by the selected LFO.
	 * 
	...
	// XXX - the offset comment is wrong
	/**
	 * Reads from the delay memory with the read pointer value modulated
	 * by the selected LFO.
	 * 
